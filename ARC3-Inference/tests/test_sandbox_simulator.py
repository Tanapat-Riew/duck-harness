...
def _initial_state(simulator_source="", history=None, no_effect_actions=None):
    if history is None:
        history = [
...
def _run(code, simulator_source="", history=None, no_effect_actions=None):
    return run_sandboxed_python(
        code=code,
        timeout_seconds=20,
        initial_state=_initial_state(
            simulator_source, history=history, no_effect_actions=no_effect_actions
        ),
        action_handler=_no_actions,
    )


def test_save_simulator_accepts_a_valid_simulator_and_returns_the_source():
    outcome = _run(f'result = save_simulator("""{CORRECT_MODEL}""")')
    assert outcome["result"]["ok"] is True
    assert outcome["result"]["defines"] == ["encode", "step"]
    assert "def step(state, action):" in outcome["simulator_source"]


def test_save_simulator_rejects_a_simulator_without_step():
    outcome = _run('result = save_simulator("def encode(frame):\\n    return 1\\n")')
    assert outcome["result"]["ok"] is False
    assert "step" in outcome["result"]["error"]
    assert outcome["simulator_source"] == ""


def test_save_simulator_rejects_a_syntax_error():
    outcome = _run('result = save_simulator("def encode(frame:\\n")')
    assert outcome["result"]["ok"] is False
    assert outcome["simulator_source"] == ""
...
def test_a_saved_model_is_reloaded_into_the_next_call():
    outcome = _run("result = encode(current_frame)", simulator_source=CORRECT_MODEL)
    assert outcome["result"] == 2
    assert outcome["error"] == ""


def test_a_stored_model_that_fails_to_reload_reports_the_error():
    outcome = _run("result = simulator_error", simulator_source="raise ValueError('stale model')\n")
    assert "stale model" in outcome["simulator_error"]
    assert outcome["simulator_source"] == ""


def test_saving_a_good_model_clears_a_stale_reload_error():
    outcome = _run(
        f'result = save_simulator("""{CORRECT_MODEL}""")',
        simulator_source="raise ValueError('stale model')\n",
    )
    assert outcome["result"]["ok"] is True
    assert "def step(state, action):" in outcome["simulator_source"]
    assert not outcome["simulator_error"]
...
def test_backtest_certifies_a_correct_model():
    outcome = _run("result = run_backtest()", simulator_source=CORRECT_MODEL)
...
def test_backtest_reports_the_first_mismatch():
    outcome = _run("result = run_backtest()", simulator_source=WRONG_MODEL)
...
def test_backtest_captures_an_exception_as_a_mismatch():
    outcome = _run("result = run_backtest()", simulator_source=EXPLODING_MODEL)
    report = outcome["result"]
    assert report["certified"] is False
...
def test_backtest_without_a_model_reports_not_ok():
    ...
    assert outcome["result"]["ok"] is False
    assert "save_simulator" in outcome["result"]["error"]


def test_backtest_with_zero_transitions_never_certifies():
    outcome = _run(
        "result = run_backtest()",
        simulator_source=CORRECT_MODEL,
...
def test_backtest_with_max_transitions_zero_reports_no_transitions():
    outcome = _run("result = run_backtest(max_transitions=0)", simulator_source=CORRECT_MODEL)
    report = outcome["result"]
    assert report["total"] == 0
    assert report["certified"] is False
...
def test_saving_a_model_clears_a_previous_backtest():
    outcome = _run(
        f'run_backtest()\nresult = save_simulator("""{CORRECT_MODEL}""")',
        simulator_source=CORRECT_MODEL,
    )
    assert outcome["result"]["ok"] is True
...
def test_a_reloaded_model_does_not_clobber_runtime_state():
    outcome = _run("result = run_backtest()", simulator_source=COLLIDING_MODEL)
...
def test_a_reloaded_model_is_isolated_from_runtime_globals_like_a_saved_one():
    outcome = _run("result = encode(current_frame)", simulator_source=RUNTIME_GLOBAL_MODEL)

...
def test_saving_a_new_source_clears_a_previous_backtest():
    agent = _agent()
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_A, backtest=CERTIFIED))
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_B))
    assert agent._simulator_source == MODEL_B
...
def test_reloading_the_same_source_keeps_its_backtest():
    agent = _agent()
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_A, backtest=CERTIFIED))
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_A))
...
def test_a_reload_error_discards_the_model_and_its_backtest():
    agent = _agent()
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_A, backtest=CERTIFIED))
    agent._apply_sandbox_simulator_result(_sandbox_result(error="NameError: nope"))
    assert agent._simulator_source == ""
    assert agent._last_backtest is None
    assert "NameError" in agent._simulator_error


def test_a_reload_error_stays_pending_across_a_later_call_without_one():
    agent = _agent()
    agent._apply_sandbox_simulator_result(_sandbox_result(error="NameError: nope"))
    agent._apply_sandbox_simulator_result(_sandbox_result())
    assert "NameError" in agent._simulator_error
...
def test_a_pending_error_is_rendered_once_and_then_forgotten():
    agent = _agent()
    agent._apply_sandbox_simulator_result(_sandbox_result(error="NameError: nope"))
    first = agent._simulator_status_lines()
    second = agent._simulator_status_lines()
...
def test_bookkeeping_survives_an_agent_pickled_before_these_attributes_existed():
    agent = ToolAgent.__new__(ToolAgent)
    agent._apply_sandbox_simulator_result(_sandbox_result(source=MODEL_A, backtest=CERTIFIED))
    assert agent._simulator_source == MODEL_A
    assert agent._last_backtest == CERTIFIED

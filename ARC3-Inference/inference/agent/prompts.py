...
STRUCTURED_RUNTIME_STATE_ADDENDUM = (
    ...
    "- `last_action_result` is the persisted result dict from the most recent `action(...)` call. It remains available across later Python inspection calls that do not call `action(...)`, and is `{}` before any action result exists. Read transition metadata from fields/keys such as `last_action_result['board_changed']`, `last_action_result['done']`, `last_action_result['level_completed']`, `last_action_result['game_over']`, `last_action_result['run_complete']`, `last_action_result['reward']`, `last_action_result['valid_actions']`, and `last_action_result['change_report']`.\n"
    "- `change_report` is a key of `last_action_result`. It summarizes the board diff bracketed by the whole `action(...)` call: `changed_count` is how many cells differ, `bounding_box` is `[min_row, min_col, max_row, max_col]` around them or `None`, and `border_only` is True when every changed cell sits in an edge strip, meaning only a HUD or timer bar moved.\n"
    "- `border_only` is `False` when nothing changed at all, so read `changed_count` first; `border_only == False` on its own is not evidence that gameplay moved.\n"
    "- `no_effect_actions` is a list of action names that produced no board change on this level so far. Positional `MOUSE` clicks are never listed there, because a specific row/col is not a reusable action identity.\n"
